...
PROCEDURE pi-tramita-manifestacao:
	...
	IF AVAIL(manifestacao) THEN DO:
        ...
        IF pTpItem = 'SAC' THEN DO:
            RUN pi-envia-email(INPUT app_config.email_remetente_solic, INPUT c-email, INPUT 'ManifestaÁ„o ' + manifestacao.nr-protocolo + ' ' + cpc-out(c-acao) + ' por ' + cUsuarioCorrente, INPUT (c-body), OUTPUT c-status, INPUT '').
        ...
        ELSE IF pConclusao = 'Procede' THEN DO:
            RUN pi-envia-email(INPUT app_config.email_remetente_solic, INPUT c-email, INPUT 'ManifestaÁ„o ' + manifestacao.nr-protocolo + ' ' + cpc-out(c-acao) + ' por ' + cUsuarioCorrente, INPUT (c-body), OUTPUT c-status, INPUT pConclusao).
            RUN pi-finaliza-tarefa-manifestcacao(INPUT INT(pIdTarManifestacao)).
...
PROCEDURE pi-finaliza-edicao-manifestacao:
	...
	IF AVAIL(manifestacao) THEN DO:
        ...
        RUN pi-envia-email(INPUT app_config.email_remetente_solic, INPUT c-email, INPUT 'ManifestaÁ„o ' + manifestacao.nr-protocolo + ' editada por ' + usuarioCorrente, INPUT (c-body), OUTPUT c-status, INPUT 'Finalizada').
...
PROCEDURE pi-transfere-manifestacao:
	...
	IF AVAIL(manifestacao) THEN DO:
		...
		ASSIGN c-email = fi-get-email-destinatario(INPUT manifestacao.cod-grp-usuar)
			   .
	    RUN pi-envia-email(INPUT app_config.email_remetente_solic, INPUT c-email, INPUT 'ManifestaÁ„o ' + manifestacao.nr-protocolo + ' ' + cpc-out(c-acao) + ' por ' + usuarioCorrente, INPUT (c-body), OUTPUT c-status, INPUT 'Transferida').
        RUN pi-finaliza-tarefa-manifestcacao(INPUT INT(pIdTarManifestacao)).
...
PROCEDURE pi-devolve-manifestacao:
	...
	IF AVAIL(manifestacao) THEN DO:
		...
		ASSIGN c-email = fi-get-email-destinatario(INPUT manifestacao.cod-grp-usuar)
			   .
	    RUN pi-envia-email(INPUT app_config.email_remetente_solic, INPUT c-email, INPUT 'ManifestaÁ„o ' + manifestacao.nr-protocolo + ' ' + cpc-out(c-acao) + ' por ' + usuarioCorrente, INPUT (c-body), OUTPUT c-status, INPUT 'Devolvida').
        RUN pi-finaliza-tarefa-manifestcacao(INPUT INT(pIdTarManifestacao)).
...
PROCEDURE pi-red-json:
   ...
   ASSIGN pRetOK = TEMP-TABLE body:READ-JSON("HANDLE", WEB-CONTEXT, "MERGE") NO-ERROR.
